...
df_m["target"] = np.sign(df_m["future_ret"] - 0.05)
...
with open("../05_model/上昇モデル/best_boosters_ver3.pkl", "wb") as fout:
...
df_test["predict"] = np.mean([clf.predict(X_test) for clf in best_model], axis = 0)
df_all_test["predict"] = np.mean([clf.predict(df_all_test[X_col]) for clf in best_model], axis = 0)
...
plt.legend()
#plt.savefig("../04_output/図示/ヒストグラム.jpg")
plt.show()
...
for code_type in df_test["17業種区分"].dropna().unique():
    ...
    plt.title(code_type)
    # plt.savefig("../04_output/図示/業界別散布図/" + "散布図_" + str(code_type) + ".jpg")
    plt.show()
#%%
# =============================================================================
# 変数重要度など
# =============================================================================
list_imp = [clf.feature_importance(importance_type = "gain") for clf in best_model]
arr_imp = np.mean(list_imp, axis = 0)
sr_imp = pd.Series(arr_imp, index = best_model[0].feature_name())
...
explainers = [shap.TreeExplainer(ml) for ml in best_model]
shap_values = [exp.shap_values(X_test) for exp in explainers]
shap_values = np.mean(shap_values, axis = 0)
shap_values = shap_values[1]
shap.summary_plot(shap_values, X_test, plot_type = "bar", show = False)
# plt.savefig("../04_output/図示/shap_summary.jpg")
plt.show()
...
#         plt.savefig("../04_output/図示/shap/shap_" + col + ".jpg")
        plt.show()
    except:
        pass
...
#%%
def score(X, y):
    y_pred = np.mean([clf.predict(X) for clf in best_model], axis = 0)
...
plt.title(top_title + "\n" + all_title + "\n" + worst_title)
plt.legend()
# plt.savefig("../04_output/図示/Simulation.jpg")
plt.show()
#%%
# =============================================================================
...
# plt.savefig("../04_output/図示/検証プロット.jpg")
plt.show()
...
# 予測結果保存
# =============================================================================
df_use["predict"] = np.mean(best_model.predict(df_use[X_col]), axis = 0)
df_use.to_csv("../04_output/予測結果.csv", encoding = "Cp932")
#%%
# =============================================================================
# ブレインパッドの予測値確認
# =============================================================================
...
for i in range(10):
    ...
    plt.barh(shap_plt.index, shap_plt)
    plt.title(df_all_test.iloc[-i, :].loc["銘柄名"] + "\n " + df_all_test.iloc[-i, :].loc["33業種区分"])
    # plt.savefig("../04_output/図示/最新のWorst/" + str(i) + "_"  + df_all_test.iloc[-i, :].loc["銘柄名"] + "_shap.jpg")
    plt.show()
