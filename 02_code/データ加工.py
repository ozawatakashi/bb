...
# =============================================================================
#col_calc_ts = ['総資産', '純資産', '株主資本', '利益剰余金', '短期借入金', '長期借入金', 'BPS',
#               '自己資本比率', '営業CF', '投資CF', '財務CF', '現金同等物', '営業CFマージン', '売上高',
#               '営業利益', '経常利益', '純利益', 'EPS', 'ROE', 'ROA', '剰余金の配当',"配当利回り",
#               '総還元性向', '純資産配当率', 'PER', 'PBR', 'ROS', 'OIM', '総資産経常利益率',
#              '総資産回転率']
#col_pct = [x + "増減率" for x in col_calc_ts]
#df_m[col_pct] = df_m.groupby("コード")[col_calc_ts].pct_change()
#df_m = df_m.dropna(subset = ["総資産増減率"])
# =============================================================================
...
for i, clf in enumerate(best_model.boosters):
    pickle.dump(clf, open("../05_model/lgb_" + str(i) + ".pkl", 'wb'))
...
if tuner_cv.best_params["objective"] == "binary":
    ...
    plt.xlabel("target")
    plt.ylabel("予測値")
    plt.savefig("../04_output/図示/散布図.jpg")
    plt.show()
...
for col in X_col:
    shap.dependence_plot(ind = col, shap_values = shap_values, features = X_test, show = False)
