...
df_m["配当性向"] = df_m["一株配当"] / df_m["EPS"] 
df_m["設備投資"] = df_m["設備投資"].abs()
df_m["売上高設備投資比率"] = df_m["設備投資"] / df_m["売上高"]
df_m["営業CF対設備投資比率"] = df_m["営業CF"] / df_m["設備投資"]
...
col_calc_ts = ["売上高", "営業利益", "経常利益"]
col_pct = [x + "伸び率" for x in col_calc_ts]
df_m[col_pct] = df_m.groupby("コード")[col_calc_ts].diff()
#col_diff = [x + "前年差" for x in col_calc_ts]
#df_m[col_diff] = df_m.groupby("コード")[col_calc_ts].diff()
...
with open("../05_model_test/上昇モデル/best_boosters.pkl", "wb") as fout:
...
plt.savefig("../04_output_test/図示/AUC.jpg")
plt.show()
...
plt.legend()
plt.savefig("../04_output_test/図示/ヒストグラム.jpg")
plt.show()
...
for code_type in df_test["17業種区分"].dropna().unique():
    ...
    plt.title(code_type)
    plt.savefig("../04_output_test/図示/業界別散布図/" + "散布図_" + str(code_type) + ".jpg")
    plt.show()
#%%
# =============================================================================
# 変数重要度など
...
plt.savefig("../04_output_test/図示/gini_importance.jpg")
plt.show()
...
plt.savefig("../04_output_test/図示/shap_summary.jpg")
plt.show()
...
for col in X_col:
    ...
    plt.savefig("../04_output_test/図示/shap/shap_" + col + ".jpg")
    plt.show()
...
df_m.to_csv("../04_output_test/予測結果.csv", encoding = "Cp932")
#%%
# =============================================================================
# ブレインパッドの予測値確認
# =============================================================================
...
for i in range(10):
    ...
    plt.savefig("../04_output_test/図示/最新の有力候補/" + str(i) + "_"  + df_all_test.iloc[i, :].loc["銘柄名"] + "_shap.jpg")
    plt.show()
...
for i in range(10):
    ...
    plt.barh(shap_plt.index, shap_plt)
    plt.title(df_all_test.iloc[-i, :].loc["銘柄名"] + "\n " + df_all_test.iloc[-i, :].loc["33業種区分"])
    plt.savefig("../04_output_test/図示/最新のWorst/" + str(i) + "_"  + df_all_test.iloc[-i, :].loc["銘柄名"] + "_shap.jpg")
    plt.show()
...
df_auc["decision"] = np.sign(df_auc["predict"] - 0.5)
...
df_auc["decision"] = np.sign(df_auc["predict"] - 0.8)
df_auc.loc[df_auc["decision"] == -1, "decision"] = 0
print("正解率 : " + str(accuracy_score(df_auc["target"], df_auc["decision"])))
print("Presicion : " + str(precision_score(df_auc["target"], df_auc["decision"])))
print("Recall : " + str(recall_score(df_auc["target"], df_auc["decision"])))
print("割合 : ", df_auc["decision"].mean()*100 , "%", sep = "")